...
class WandoujiaDownloaderMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        if request.url.endswith('/top/app'):
            driver = webdriver.PhantomJS(executable_path = settings.PHANTOMJS_PATH) 
            driver.get(request.url)
            # 执行js的方法会有问题，所以用selenium的方法来执行
            # driver.execute_script(load_more_js)   
            # time.sleep(50)  # 等待JS执行
            #此处逻辑是判断加载的应用数目有多少，低于200的话就一直点“查看更多”按钮
            while True:
                apps = driver.find_elements_by_css_selector('#j-top-list .card')
                print "=====", len(apps)
                if len(apps) >= 200:
                    break
